...
16. **`SEND_SMS` is declared, not always requested.** The manifest ships the permission so `autoSendSms` *can* request it at runtime, but the request only fires when the user flips the auto-send toggle AND the panic button actually triggers. Users who never opt in never see the SEND_SMS dialog. Do not move the `Permission.sms.request()` earlier (e.g. into onboarding) — it scares users who don't need it and triggers Play Store restricted-perm review pipelines if the app ever leaves sideload distribution.
