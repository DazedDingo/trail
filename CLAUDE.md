...
│   ├── import_timeline_screen.dart # Google Maps Timeline import flow: pick → preview + preset → import with progress → undo / coverage offer (0.16.0); imports sheet with Undo / Map detail… / Photos… per import; "Use as home location" from Google's HOME (0.17.0)
│   ├── places_screen.dart       # Places from imported Google visits: counts, first/last, time spent, visit sheet → map by day (0.17.0; pure maths in services/stats/places_service.dart)
