...
│   ├── stats_settings_provider.dart # statsIncludeImportsProvider (AsyncNotifier<bool>, prefs `trail_stats_include_imports_v1`, default false) — allPingsProvider watches it (0.17.0)
│   └── places_provider.dart     # placesProvider / placeVisitsProvider over PingDao.importedVisits (0.17.0)
...
│   ├── date_labels.dart         # The pin/HUD/caption date formats, all with the year (0.16.2) — see feedback: never "11 Aug", always "11 Aug 2024"
│   ├── panic/
...
│   │   ├── import_provenance.dart      # describeImportNote: `gmaps:*` note → "Imported from Google Timeline · visit · Home" (0.16.2)
...
│   │   └── coverage_flow.dart          # runCoverageFlow: the ONE plan → confirm → progress → install UI flow (Settings button, import finish, Timeline imports → Map detail…) (0.16.1)
│   └── tiles/
...
│   ├── history_screen.dart      # Full history: year chips (YearChipsRow) + month headers + keyset paging per year (`PingDao.pageByRange`, 0.17.0); imports included
...
│   ├── import_timeline_screen.dart # Google Maps Timeline import flow: pick → preview (+ per-year in-file / kept / duplicate table and the file's full date range, also for already-imported files — 0.17.5) + preset → import with progress → undo / coverage offer (0.16.0); imports sheet with Undo / Map detail… / Photos… per import; "Use as home location" from Google's HOME (0.17.0)
│   ├── places_screen.dart       # Places from imported Google visits: counts, first/last, time spent, visit sheet → map by day (0.17.0; pure maths in services/stats/places_service.dart — visits cluster within ~120 m and rows with the same geocoded name within 1 km merge, 0.17.5)
...
│   ├── year_chips_row.dart      # Shared year-chip row (All/years/Older…) used by the date filter panel and History (0.17.0)
│   ├── inline_date_filter_panel.dart # Preset chips + year chips (from `pingYearsProvider` → `yearsCovering`; tap = `rangeForYear`, 0.16.1) + custom range picker used by the map panel
│   └── stats/
...
22. **Pins are native style layers; never null `_controller` for a data or range change.** (0.14.0 replaced the annotation API — `addCircle` per pin was Θ(N²) in maplibre_gl 0.26.0 because `AnnotationManager._setAll` re-serialises every annotation on each add; see `docs/PERF_PLAN.md` §1.) The model now:
    ...
    - Data goes up once per provider list identity: `buildPinSnapshot` → typed `PinColumns` (incl. `srcs`: 0 live / 1 imported, 0.16.2) → `Isolate.run(buildPinsGeoJson/buildSegmentsGeoJson)` (inline below 256 fixes) → `editGeoJsonSource(id, String)`. Each pin feature carries `i`, `id` and `s`; `buildPinStyle` makes `s == 1` pins hollow (opacity 0.15 + teal stroke) with the head/prev emphasis evaluated FIRST so the cursor pin is never dimmed — the only 0.26.0 upload that takes a pre-serialised string. `_uploadGen` is bumped by `_invalidateUpload()`; every await re-checks it, so a superseded upload drops itself.
...
32. **Release builds are R8-shrunk and per-ABI (0.15.0) — a one-way door.** `--split-per-abi` makes Flutter's Gradle plugin set `versionCode = abiIndex × 1000 + build` (armeabi-v7a 1, arm64-v8a 2, x86_64 4), so the arm64 build of `+97` installs as 2097. A future fat APK with a plain versionCode would be rejected as a downgrade on every existing install — if the split is ever reverted, add the offset by hand. Keep rules live in `android/app/proguard-rules.pro` (Gson for flutter_local_notifications, Tink for flutter_secure_storage, `com.dazeddingo.trail.**`); SQLCipher / MapLibre / WorkManager / file_picker ship theirs in their AARs. Dart obfuscation stays off (readable stack traces beat a few MB). Toolchain (0.17.2): **AGP 9.1.0, Gradle 9.3.1, Kotlin 2.4.0, Flutter 3.47.1** — exactly Flutter's template (`gradle_utils.dart`: `templateAndroidGradlePluginVersion` / `templateKotlinGradlePluginVersion` / `templateDefaultGradleVersion`). `android/gradle.properties` carries the template's `android.newDsl=false` and `android.builtInKotlin=false`; the app no longer applies `kotlin-android` itself (`kotlin { compilerOptions { jvmTarget } }` stays). Because built-in Kotlin is OFF, plugins that still apply `kotlin-android` are only a warning — BUT Flutter decides which plugin modules get the Kotlin plugin by regex-scanning their build scripts, and plugins that guard it with `if (agpMajor < 9)` are skipped, so `android/build.gradle.kts` has a `subprojects { plugins.withId("com.android.library") { apply kotlin-android } }` block; without it maplibre_gl fails (`kotlin()` not found) and file_picker compiles no Kotlin. Don't remove it. `share_plus` 10.x and `package_info_plus` 9.x are pinned on purpose: their newer majors pull `win32 6` → `flutter_secure_storage 11`, whose changelog says data saved with the old algorithms becomes unusable — that is the SQLCipher DB key (the "persist APIs on upgrade" rule). Moving them needs a staged `flutter_secure_storage` 9 → 10 (cipher migration) → 11 plan with a real-device check. Tests: `Workmanager()`'s constructor resets `WorkmanagerPlatform.instance` to the host implementation, and on Linux `workmanager_linux` then writes REAL systemd user timers under `flutter test` — any test touching `WorkmanagerScheduler.*` must construct `Workmanager()` once BEFORE installing its fake (see the two scheduler tests). Two environment facts: this arm64 VPS **cannot** run `flutter build apk` (Flutter's x86-64 `gen_snapshot` segfaults under QEMU — build on CI only), and Flutter 3.41's `MinSdkVersionMigration` rewrites `minSdk = 23` → `flutter.minSdkVersion` (24) in the working copy on every build, CI included — the committed 23 is nominal; the shipped APKs are effectively minSdk 24.
...
36. **Auto-coverage (`lib/services/coverage/`) is prefs-driven so the worker stays DB- and network-free.** `CoveragePrefs` holds the installed-archive extents (written by the UI isolate after probing), the pending-points queue (5 km dedupe, cap 200), settings (enabled + Wi-Fi-only, both default true), server URL (prefs) and token (secure storage). The WorkManager tick calls `CoverageService.noteFixInWorker(lat, lon)` after a real fix: `isCoveredInDetail` (some archive with maxZoom ≥ 12 whose bounds contain the point; null bounds = not covered) else `addPending`. `CoverageResumeObserver` (post-frame + `onResume`, throttled 10 min) runs `processPendingOnAppOpen`: `shouldAutoFetchNow` gate → `refreshExtents` → `planCoverage` (greedy centroid 15 km → pad 3 km → merge overlaps → drop boxes already covered) → dry-run sizing against the server → auto-fetch ≤ 20 MB, otherwise re-queue + a one-line notice on the Settings tile; then (0.17.0) once per 7 days (`trail_coverage_last_refresh_v1`) it re-fetches up to 2 coverage packs whose file-name date is > 180 days old (`selectStalePacks`, oldest first, same bbox/zooms from the probe, skipped when the server's planet date equals the pack's) inside the same byte cap, deleting the old file only after the new one landed. Downloads land in `<docs>/tiles/coverage-<slug>-z7-14-<date>.pmtiles`; the `coverage` name is what gives them their role (gotcha 31), so the multi-archive loopback serves them with no further registration — just `invalidateTileProviders`. The server contract (`GET /v1/health`, `/v1/extract?bbox&minzoom&maxzoom[&dry_run=1]`, bearer token, 413 above 1 sq deg) is `tools/coverage/server.py`; the phone never contacts Protomaps. `TileDownloader.download` now takes `headers` and honours `Content-Disposition`. **UI entry points all go through `runCoverageFlow` (0.16.1)** — it owns the confirm dialog (`N areas · MB · planet`), the progress `DialogRoute` (removed via `removeRoute`, never pop-by-captured-context; the earlier copy stranded the dialog when a fetch finished before first build and threw on a disposed `ValueNotifier`) and the explicit "server not set → Open Settings" note. Never re-implement that dialog pair in a screen; add a `CoverageFlowStatus` branch instead. A missed post-import offer is recoverable per import from *Timeline imports → Map detail…* (`PingDao.fixesByImportId`, stride-sampled to 5 000).

37. **Secure storage migration (0.17.3 = step 1, flutter_secure_storage 9 → 10; step 2 = 11).** The shared instance keeps `encryptedSharedPreferences: true` (deprecated, but it is what tells 10.x to migrate the Jetpack/Tink store), `migrateOnAlgorithmChange: true`, **`resetOnError: false`** (10.x defaults it to TRUE — "wipe everything on any decrypt error" — with the SQLCipher key in there that is never acceptable), and the 11-compatible ciphers (RSA-OAEP key wrap + AES-GCM). `SecureStorageMigration.verifyAndRewrite` runs post-first-frame: reads every key in `knownKeys`, writes the same value back (forces the new cipher), re-reads, and on full success writes SharedPreferences `trail_secure_storage_v10_ok_v1`; Diagnostics shows it. **Step 2 shipped in 0.17.4 (fss 11.0.0, share_plus 13, package_info_plus 10, file_picker 12, win32 6, compileSdk 37):** the deprecated option is gone; `computeStartupKeyState` returns `notMigrated` when `trail.db` exists, no salt, the key reads null AND the marker is absent (11 reads legacy data as silent null, never an exception) → `/recover` "Update needed first" variant (link to v0.17.3-105, Try again only — no destructive buttons); a working install without a marker gets `markVerified` and proceeds. The 0.17.3 tag is hard-coded in `key_recovery_screen.dart` and pinned by a test. Keep both gates (`keyMissing`, `notMigrated`) — they mean different things. Independent of the library: `KeystoreKey.getOrCreate` never creates a key while `trail.db` exists (`KeyMissingException` → `/recover`: Try again / Use backup passphrase / Start a new log, which sets the file aside as `trail.db.locked-<stamp>`, never deletes). Add any new secret to `knownKeys` or it will not be verified. Facts from the 10.3.1 Java worth keeping: (a) if the Jetpack MasterKey is gone, `initialize()` logs "Falling back to custom ciphers" and every read returns null — the 9.x values stay in `FlutterSecureStorage.xml` unreadable; that is the `keyMissing` state and "Try again" will keep failing there; (b) `migrateWithBackup` is deliberately left `false` — it routes through a separate, less-travelled migration path and our verify-and-rewrite + marker is the safety net; (c) the prefs file name and key prefix are byte-identical between 9.2.4 and 10.3.1 (pinned by `secure_storage_test`) — never set `storageNamespace`, it would orphan every existing install; (d) `resetOnError: true` literally calls `deleteAll()` on a read failure.

38. **The DB key has two homes (0.17.7): secure storage AND a Trail-owned escrow.** `KeystoreKey.read()` reads flutter_secure_storage first; on a THROWN read (plugin init/Keystore failure) or a silent null it falls back to `KeyEscrow.instance.load()` (Kotlin `KeyEscrowPlugin`, channel `trail/key_escrow`, AES-256-GCM key in AndroidKeyStore alias `com.dazeddingo.trail.db_key_escrow.v1`, ciphertext + sha256 fingerprint in prefs file `trail_key_escrow`), records `lastReadSource` / `lastSecureStorageError`, and best-effort re-persists into secure storage. A thrown read with no escrow copy RETHROWS (a swallowed throw would look like a fresh install to `getOrCreate`). `TrailDatabase._openWithKey` mirrors the key into the escrow after a successful open only (never escrow an unverified key; skipped when the fingerprint matches). The escrow channel is registered by MainActivity, so it exists in the UI isolate only — the WorkManager isolate logs `key_unavailable` and skips the tick when secure storage fails there. Never set `setUserAuthenticationRequired` or StrongBox on the escrow key. Incident that motivated it: 2026-08-23, fss 11 `Failed to unwrap key` ← `KeyStoreException UNKNOWN_ERROR (-1000)` on a launch after the upgrade; the plugin's own `createRSAKeysIfNeeded` regenerates the RSA pair if the Keystore ever reports it missing, which would orphan every secret — hence also (0.17.7) `trail_startup_blocked_v1`: the worker does not touch secure storage while the last startup failed.
