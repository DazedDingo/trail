...
│   ├── regions_screen.dart      # Offline tiles library: install (.pmtiles picker), delete, set-active. PMTiles since 0.7.2+28.
...
├── widgets/
│   └── trail_map.dart           # MapLibreMap + PolylineLayer/CircleLayer + recenter button (vector renderer, 0.7.2+28)
├── theme/
...
8. **`libsqlite3.so` loader workaround stays for `ping_dao_test.dart`.** Even after the 0.7.2+28 PMTiles cutover (which removed the `flutter_map_mbtiles` dep that previously pulled in this requirement), the encrypted-DB DAO tests still hit `sqflite_common_ffi`, which unconditionally calls `DynamicLibrary.open('libsqlite3.so')`. The unversioned symlink is only in `libsqlite3-dev`, missing on CI and fresh arm64 dev images. `test/ping_dao_test.dart` and `test/archive_service_test.dart` register an `open.overrideFor(OperatingSystem.linux, ...)` callback via a **top-level function** (it's serialized across `Isolate.spawn`); the override must be installed *inside* the background isolate. If you add a new SQLite-backed test, reuse that pattern.
