...
22. **`PathRenderStrategy.fromScratch` whenever the date filter changes.** Stale annotations from the previous filter were leaking onto the map because the MapLibreMap remounts on a `pingsByRangeProvider` family-key change (loading flicker), and `_renderedCircles` / `_pathRenderKey` survived the remount. `_resetAnnotationTrackingOnFilterChange` (called from both `_applyDateFilter` and the closed-form refresh path) drops every piece of annotation tracking state, and the new pure helper `choosePathRenderStrategy` returns `fromScratch` whenever `prev.first` and `visible.first` aren't `identical` (different DAO reads = different `Ping` instances even for the same row). Don't reintroduce the inline `samePrefix && length == prev.length → noOp` short-circuit without restoring the identity guard — a same-length-different-identity collision used to render an empty map.

23. **Sub-1× playback speeds clamp the timer interval at 4 s ceiling.** `playbackInterval(baseStep, speed)` caps the result at `[16ms, 4000ms]` — the floor matches one display frame (faster speeds don't queue overlapping callbacks); the ceiling lets 0.25× on a 350ms base step render at 1400ms without bound-clipping. The `speed=0` defensive branch collapses to 1× rather than infinite-loop. `kPlaybackSpeeds = [0.25, 0.5, 1, 2, 4, 8, 16]` is the canonical cycle; `nextPlaybackSpeed` wraps past 16× to 0.25×. `formatPlaybackSpeedLabel` has an explicit aliasing guard for 0.25× — `toStringAsFixed(1)` would render as `0.3×`, indistinguishable from a hypothetical 0.3× entry.
