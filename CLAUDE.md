...
- **Map viewer:** `maplibre_gl` 0.27.0 (vector renderer, the older
  ...
  archives under `Settings → Offline map → Regions`. Since 0.15.0 every
...
│   └── tiles/
│       ├── tile_archive.dart    # TileArchive (minZoom/maxZoom/bounds/vectorLayers/mayContain/tile) + MbtilesArchive (sqflite, TMS flip) + PmtilesArchive (package:pmtiles) + tileIntersectsBounds/unionBounds
...
- **`docs/TILES.md`:** how to build archives (planetiler, OpenMapTiles schema), roles, and what the loopback does with them.
