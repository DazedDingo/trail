...
/// Dart face of the Kotlin escrow handler (`trail/key_escrow`) — Trail's
/// own second copy of the SQLCipher DB key.
///
...
/// throw is the failure mode we are hedging against. [store] and [clear]
/// are explicit user/caller actions and do surface their platform errors.
///
/// Since 0.17.9 the handler lives in the `trail_secure_store` plugin
/// package rather than in `MainActivity`, so it is registered in the
/// WorkManager background engine too (`GeneratedPluginRegistrant` runs
/// for every `FlutterEngine`). The `MissingPluginException` branch stays
/// — it is still the right answer for an engine that never registered
/// plugins at all, and for unit tests.
class KeyEscrow {
  /// The platform channel. Handler is installed by
  /// `TrailSecureStorePlugin.onAttachedToEngine`.
  ...
  Future<EscrowResult> load() async {
    try {
      ...
      return EscrowResult(error: describeStoreError(e));
    }
  }
  ...
  Future<EscrowStatus> status() async {
    try {
      ...
      return EscrowStatus(error: describeStoreError(e));
    }
  }
  ...
  /// Keystore decrypt.
  ///
  /// Never throws and is never awaited on the open path.
  ...
  /// Hex SHA-256 of the key's UTF-8 bytes — the same digest the Kotlin
  /// side writes next to the blob.
