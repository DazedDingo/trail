...
### Phase 5 — Scheduling hardening + archive — **shipped 0.5.0+14 (2026-04-20)**
Dual-path scheduler. Battery-saver mode (default) is the existing WorkManager periodic job; Precise mode uses `AlarmManager.setExactAndAllowWhileIdle` with a `SCHEDULE_EXACT_ALARM` / `USE_EXACT_ALARM` permission pair and a per-ping self-rescheduling receiver chain (`ExactAlarmScheduler` → `ExactAlarmReceiver` → WorkManager one-off → next alarm). Switching mode from `Settings → Scheduling → Mode` cancels the other side's work first; denying the API-31+ runtime permission deep-links to `Settings.ACTION_REQUEST_SCHEDULE_EXACT_ALARM` and aborts the mode switch instead of silently degrading. `BootReceiver` re-arms the exact alarm if the persisted mode is `exact`, so the chain survives reboots and APK upgrades with no app open. Last-20 scheduler events (native `SchedulerEventsLog`) surface on the same settings card for "yes, the alarm actually fired at 02:07" evidence. Archive flow (`Settings → History → Archive older pings`): date-cutoff picker, GPX+CSV/gpx/csv format selector, row-count + earliest/latest preview, destructive confirm dialog, export-then-delete (never delete if any export write throws), `share_plus` hand-off. **Demo:** toggle scheduler modes, watch the events log grow with EXACT_SCHEDULED / EXACT_FIRED as the cadence runs; archive a year of pings out to a Drive-saved file and confirm the DB shrunk.

### Phase 6 — Polish — **shipped 0.6.0+15 (2026-04-20)**
Diagnostics screen at `Settings → Diagnostics`: parallel-loaded permission matrix (location / background location / battery optimisation / notifications / exact-alarm), DB integrity check (`PRAGMA integrity_check` button that surfaces the row list inline), last-20 worker runs sourced from `WorkerRunLog` (a shared-prefs rolling log written by every `_callbackDispatcher` branch so the UI isolate can see what the WorkManager isolate did without adb), and a "copy all" AppBar action that bundles the whole snapshot to clipboard for pasting into a bug report. Date-range export: the two home-screen "Export GPX" / "Export CSV" buttons (which dumped ALL history) collapse into one "Export…" entry opening a dialog with `showDateRangePicker` + GPX+CSV / GPX-only / CSV-only radio; "All history" remains a one-tap preset. Date filter is factored to a pure `filterPingsByRange` function (inclusive start, end bumped to next-day midnight so single-day picks cover the whole local day). Heatmap overlay on the map screen: AppBar toggle swaps the path+pins for a 0.001°-grid (~100 m at equator) bucketed marker layer rendered with RadialGradient-filled circles, alpha scaled to normalised bucket density. Home location feature: `Settings → Home` stores lat/lon/label in shared prefs; setter screen offers "use last successful fix" or manual entry; home-screen shows "X km from home" under the current coords via Haversine distance. Adaptive icon (mipmap-anydpi-v26) already in place with neutral teal Material-pin foreground on dark background — matches PLAN's "neutral / disguised" requirement, no change needed. **Demo:** flip heatmap toggle on map screen to see density hot-spots instead of the path polyline; open diagnostics and `PRAGMA integrity_check` returns `['ok']`; set home from last fix, then the home screen shows "0.2 km from home" next to current coords.
