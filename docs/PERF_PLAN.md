...
### Implementation notes (0.15.0+97) — M5 + §3 #10, #12, #20 + coverage Phase A

- **M5 `maplibre_gl` 0.27.0:** zero source changes needed (`copyWith` unused; style content already lives in `onStyleLoadedCallback`). Added `MapLibreMap.preWarm()` in the post-first-frame block (gotcha 30 — nothing new is awaited before `runApp`). `addHeatmapLayer` still has no `filter` parameter in 0.27.0, so the follow-up `setFilter` stays (gotcha 12). `pmtiles` pinned at 2.0.0: 2.1+ wants `meta ^1.18`, which `flutter_test` on Flutter 3.41.6 pins below.
- **#20 R8 + per-ABI APKs:** `isMinifyEnabled` + `isShrinkResources` with `proguard-android-optimize.txt` + `android/app/proguard-rules.pro` (Gson keeps copied from flutter_local_notifications' own example, Tink keeps for flutter_secure_storage's `KeyTemplates` reflection, blanket keep on `com.dazeddingo.trail.**`; SQLCipher, MapLibre, WorkManager and file_picker ship consumer rules in their AARs). CI builds `--split-per-abi`, verifies the pinned SHA-1 on all three APKs and uploads `Trail-<tag>-{arm64-v8a,armeabi-v7a,x86_64}.apk`. Flutter's Gradle plugin sets `versionCode = abiIndex × 1000 + build` (armeabi-v7a 1, arm64-v8a 2, x86_64 4 — `FlutterPlugin.kt:623`), so 0.15.0+97 on an arm64 phone is versionCode 2097. **One-way door:** a future fat APK with a plain versionCode would be a downgrade. Dart obfuscation deliberately off. Honest note: this arm64 VPS cannot run Flutter's x86-64 `gen_snapshot` under QEMU (SIGSEGV), so R8 ran for the first time on CI, not locally. Previous fat APK: 108 MB.
- **#12 worker:** `_handleBoot` passes its open handle into `_handleScheduled({db})` (one SQLCipher KDF per boot tick); `enqueuePeriodic` records `trail_scheduler_last_enqueued_min_v1` and the tick only re-registers when `SchedulerPolicy.shouldReenqueuePeriodic` says the effective cadence changed; `SchedulerPolicy.shouldAutoFetchPhotos(networkState, isCharging)` gates the in-worker Wikimedia fetch to wifi/ethernet, or mobile-while-charging (`none`/`unknown` never try, saving the 2 × 8 s timeouts). Photos for other pings arrive via the manual backfill sheet.
- **#10 tile server:** glyph/sprite asset bytes memoised after first `rootBundle.load`; every `response.close()` awaited and caught.
- **Coverage Phase A (docs/TIMELINE_IMPORT.md §3):** `LocalTileServer.start(List<String>)` serves an ordered archive list (coverage → active region → world overview) through `TileArchive` (`MbtilesArchive` via sqflite, `PmtilesArchive` via `package:pmtiles` — `.pmtiles` regions render for the first time since 0.8.0). On a miss the server **overzooms server-side**: nearest ancestor in any archive → `overzoomMvt` (hand-rolled MVT codec + Liang–Barsky / Sutherland–Hodgman clipping, `Isolate.run`) → gzip → LRU. Style source `minzoom`/`maxzoom` are rewritten to the served union (`volatile` was tried and dropped — the style-JSON converter ignores it; see CLAUDE.md gotcha 17); the server rebinds on a fresh port whenever the archive set changes (that port change is the cache invalidation and the `_MapHost` remount). Roles live in `trail_tiles_roles_v1`. 170+ new unit tests; the one thing `flutter_test` cannot prove is that MapLibre renders the overzoomed tiles — device soak pending.
...
| 12 ✔ 0.15.0 | Background worker | `_handleBoot` opens+closes the DB then `_handleScheduled` opens again → two SQLCipher KDF runs per boot (`workmanager_scheduler.dart:449-470`); periodic task re-registered with `update` policy on **every** tick (`:280-286`); `WorkerRunLog.record` does read→decode→encode→write per tick; Wikimedia fetch inside the worker with 2 × 8 s timeouts and no backoff (`:326`, `online_photo_service.dart:47,87,112`) | Pass the handle through; re-enqueue only when cadence changed; gate photo fetch on unmetered/charging | Battery per tick; ~100–300 ms per reboot | M |
...
| **0.15.0 ✔ shipped** | M5 (`maplibre_gl` 0.27.0) + §3 #20 (R8, split-per-abi) + #12 + #10 + map-coverage Phase A (multi-archive loopback, `.pmtiles`, server-side overzoom) | Dependency + build-config + map-stack changes in one version so a regression is easy to bisect; one device soak covers all of them |
| opportunistic | #11 (style memo landed in 0.15.0; the rest), #16, #21, #22 | — |
